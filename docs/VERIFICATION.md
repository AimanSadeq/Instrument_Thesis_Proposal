...
| Screenshots of every Arabic screen on a phone, including a failed submission | **Pass, on emulated phones.** `docs/screenshots/` holds every Arabic screen at two phone sizes: consent, consent options close up, confirmation after each choice, pre-training questionnaire, daily reflection, daily reflection with Day 4 selected, and the post-training evaluation. These are Chromium at phone viewports, which is not the same as Safari on a physical iPhone; **O1** covers that. |
